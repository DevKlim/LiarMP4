...
function App() {
    ...
    const autoScore = async () => {
        if (!manualLink) return alert("Link is required to auto-score.");
        ...
        try {
            const res = await fetch('/api/score', {
                ...
                body: JSON.stringify({ link: manualLink, caption: manualCaption })
            });
            ...
            // Map agent scores to UI slider values
            setManualScores({
                visual: data.agent_scores.visual?.score || 5,
                audio: data.agent_scores.audio?.score || 5,
                source: data.agent_scores.source?.score || 5,
                logic: data.agent_scores.logic?.score || 5,
                emotion: data.agent_scores.emotion?.score || 5,
                va: data.agent_scores.video_audio?.score || 5,
                vc: data.agent_scores.video_caption?.score || 5,
                ac: data.agent_scores.audio_caption?.score || 5,
                final: data.final_veracity_score || 50
            });

            // Auto-populate tags if available
            if (data.tags && data.tags.length > 0) {
                setManualTags(data.tags.join(', '));
            }

            alert("Auto-scoring complete! Review and adjust scores if needed, then click 'Save & Add to GT'.");
        } catch (e: any) {
            console.error("Auto-score error:", e);
            alert("Auto-scoring failed: " + e.message);
        } finally {
                            ...
                            <div className="flex-1 bg-slate-900/50 border border-slate-800 rounded-xl p-6 overflow-y-auto">
                                <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-800">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            onClick={autoScore}
                                            disabled={isAutoScoring || !manualLink}
                                            className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold flex gap-2 disabled:opacity-50 hover:bg-indigo-700 transition">
                                            {isAutoScoring ? (
