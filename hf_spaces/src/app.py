        ...
        for item in items:
            ...
            if res_data and "parsed_data" in res_data:
                ...
                if task_type == "Verify" and gt_data is not None:
                     ...
                     yield f"data:      Overall Veracity   : AI {ai_score} | GT {gt_final}\n"
                     yield f"data:      Accuracy Delta     : {delta} points\n\n"
                ...
                row = {
                    ...
                    "visual_score": parsed.get('veracity_vectors', {}).get('visual_integrity_score', 0),
                    "audio_score": parsed.get('veracity_vectors', {}).get('audio_integrity_score', 0),
                    "source_score": parsed.get('veracity_vectors', {}).get('source_credibility_score', 0),
                    "logic_score": parsed.get('veracity_vectors', {}).get('logical_consistency_score', 0),
                    "emotion_score": parsed.get('veracity_vectors', {}).get('emotional_manipulation_score', 0),
                    "align_video_audio": parsed.get('modalities', {}).get('video_audio_score', 0),
                    "align_video_caption": parsed.get('modalities', {}).get('video_caption_score', 0),
                    "align_audio_caption": parsed.get('modalities', {}).get('audio_caption_score', 0),
                    "classification": parsed.get('disinformation_analysis', {}).get('classification', 'None'),
                    "reasoning": parsed.get('final_assessment', {}).get('reasoning', ''),
                    "tags": ",".join(parsed.get('tags',[])),
                    "raw_toon": res_data.get('raw_toon', ''),
                    "config_type": "GenAI Inference",
                ...
                try:
                    with open(d_path, 'a', newline='', encoding='utf-8') as f:
                        writer = csv.DictWriter(f, fieldnames=DATASET_COLUMNS, extrasaction='ignore')
                        if not exists or d_path.stat().st_size == 0: writer.writeheader()
                        writer.writerow(row)
                except Exception as e:
                    logger.error(f"Failed to write to dataset.csv: {e}")
                    
                json_data = parsed.copy()
                json_data["raw_toon"] = res_data.get('raw_toon', '')
                json_data["prompt_used"] = res_data.get('prompt_used', '')
                json_data["fcot_trace"] = res_data.get('fcot_trace', {})
                json_data["meta_info"] = {
                    "id": tid,
                    "timestamp": row["timestamp"],
                    "link": link,
                ...
                json_path = Path(f"data/labels/{tid}_{ts_clean}.json")
                try:
                    with open(json_path, 'w', encoding='utf-8') as jf:
                        json.dump(json_data, jf, indent=2, ensure_ascii=False)
                except Exception as e:
                    logger.error(f"Failed to write JSON label: {e}")

                update_queue_status(link, "Processed", task_type)
                yield f"data: [DONE] Saved analysis for {link}.\n\n"
            else:
                log_queue_error(link, "Inference Failed / No Parsed Data", task_type)
                yield f"data: [ERROR] Inference failed or no parsed data returned for {link}.\n\n"

        yield f"data: [SYSTEM] Queue processing complete.\n\n"
